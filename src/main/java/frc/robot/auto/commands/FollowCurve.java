...
import frc.robot.utils.kade.Curve;
import frc.robot.subsystems.sensors.Pigeon;
import edu.wpi.first.wpilibj2.command.Command;
import frc.robot.utils.kade.RobotPath;
...
public class FollowCurve extends Command {
    ...
    private RotationalPIDController rotPID;
    ...
    public FollowCurve(Curve curve, double targetRot, double maxVelMove, double maxVelRot) {
        ...
        this.rotPID = new RotationalPIDController(0.35, 0.015, 0.08, 0.035, 0.1, maxVelMove);
    }

    public FollowCurve(Curve curve, double targetRot, double maxVelMove, double maxVelRot, double rotDead) {
        this.path = new RobotPath(curve.getPoints(), targetRot);
        this.movePID = new PIDController(1.75, 0.025, 0.1575, 0.0, 0.0, maxVelMove);
        this.rotPID = new RotationalPIDController(0.35, 0.015, 0.08, rotDead, 0.1, maxVelMove);
    }

    public FollowCurve(Curve curve, double targetRot) {
        this(curve, targetRot, 1.0, 0.3, 0.075);
    }
    ...
    public void initialize() {
        ...
        this.rotPID.setDest(path.getTargetRot() + Math.PI / 2.0);
    }
    ...
    public void execute() {
        ...
        double moveOutput = movePID.updateOutput(
            (path.getPathLength() - path.getRemainingPathLength()) / path.getPathLength()
        );
        double rotOutput = rotPID.updateOutput(Pigeon.getRotationRad());
